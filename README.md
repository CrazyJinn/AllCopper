...
flowchart TB
    ...
    S3O1 & S4O1 & S4O2 & S4O3 & S4O4 --> 文生图api
...
| **场景设计** | 根据场景需求，生成大地图、游戏场景、对话背景、UI 背景四类场景的 AI 绘图提示词（油画风格） |
| **解决方案设计** | 基于代码需求进行技术方案分析，输出需求分析文档和测试用例设计，为代码生成提供输入 |
| **文生图 (api)** | 调用seedream API，根据文本提示词生成角色设计图和场景图片 |
| **图生图 (api)** | 调用seedream API，基于设计图 + 提示词生成角色立绘和过场图片 |
| **图生视频 (api)** | 调用seedance API，基于 Q 版设计图生成角色动画视频 |
