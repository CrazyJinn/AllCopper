...
export class HeroComponent extends Component {
    ...
    /** 鼠标屏幕位置 */
    private mouseScreenPos: Vec3 = new Vec3();
    ...
    private onMouseMove(event: EventMouse) {
        // 保存鼠标屏幕位置
        this.mouseScreenPos.x = event.getUILocationX();
        this.mouseScreenPos.y = event.getUILocationY();
    }
    ...
    private updateMouseWorldPosition() {
        ...
        if (this.camera) {
            // 使用保存的鼠标屏幕位置
            this.tempVec3.x = this.mouseScreenPos.x;
            this.tempVec3.y = this.mouseScreenPos.y;
            this.tempVec3.z = 0;
            // 转换为世界坐标
            this.camera.screenToWorld(this.tempVec3);
            inputManager.setMouseWorldPosition(this.tempVec3.x, this.tempVec3.y);
        }
    }
    ...
    private playAction(actionName: string, forceDirection?: Direction) {
        ...
        if (frames && frames.length > 0) {
            ...
            if (fallbackFrames && fallbackFrames.length > 0) {
                ...
                console.warn('[HeroComponent] 未找到动画帧: ' + cacheKey + ', 保持当前帧');
                this.currentFrames = [];
            }
        }
    ...
    private showCurrentFrame() {
        if (this.sprite && this.currentFrames[this.frameIndex]) {
            ...
            // 根据角色朝向翻转 sprite
            if (this.controller) {
                const scale = this.controller.facingX;
                this.node.setScale(scale, 1, 1);
            }
        }
    }
