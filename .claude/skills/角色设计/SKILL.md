...
| 立绘/*.png | S8 i2i(api) | 基于立绘提示词+角色设计图生成 |
| 设计图.png | S6 t2i(人工) | 基于设计图提示词生成 |
| 动画/*.png | S11 精灵帧提取(ffmpeg) | 从Q版动画视频提取精灵帧 |
| Q版/设计图.png | S9 i2i(人工) | 基于设计图.png转换 |
| Q版/动画.mp4 | S10 i2v(api) | 基于Q版设计图+动画提示词生成 |
| Q版/精灵图集.png | S11 精灵帧提取(ffmpeg) | 从动画视频提取 |
...
1. 读取 `99_变更管理/流程数据/workflow.yaml`，找到 `skill: 角色设计` 的节点，**只处理自己的节点**，不触碰其他节点
2. 读取 `01_需求文档/游戏基本信息.memory`，提取世界观背景、美术风格
3. 读取 `01_需求文档/角色需求.md`，提取角色信息、动画需求
4. **为每个角色创建独立文件夹**（按角色ID命名）
5. 在各角色文件夹内创建目录结构：
   ```
   char_001/
   ├── 设计图提示词.md    # 本skill生成
...
7. **更新 `角色设计总览.md`**，为每个角色创建独立区块
8. 验证所有输出文件是否生成成功
9. **更新 workflow.yaml**（验证通过后执行）：
   - 将本节点（角色设计）的 `status` 更新为 `completed`
   - 将本节点的 `completed_at` 更新为当前时间
   - 清空本节点的 `todo_list: []`
   - 遍历本节点的 `successors`（t2i人工），将 `status` 为 `pending` 的下游节点更新为 `ready`，并设置 `ready_at` 为当前时间
   - 根据实际生成的输出内容，更新下游节点的 `todo_list`：
     - **t2i人工**: `"根据设计图提示词生成角色设计图"`
   - 更新 `updated_at` 为当前时间
